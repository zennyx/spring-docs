...
TIP: 在许多操作系统上，Maven 可以通过包管理器进行安装 ^※2^ 。如果你使用 OSX Homebrew，试试 `brew install maven` 命令。 Ubuntu 的用户可以执行 `sudo apt-get install maven` 。而使用 https://chocolatey.org/[Chocolatey] 的 Windows 用户可以通过提权的（管理员）命令提示符执行 `choco install maven` 。
